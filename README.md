...
| 布局 | 横轴 / 纵轴 | 适合 |
|---|---|---|
| **流程图** | 自动布局（dagre） | 看整体拓扑、分支、汇聚 |
| **泳道图** | 横 = 时序（按步序分列）<br>纵 = 层（前端/后端/资源层） | 看“谁在什么时候调了谁”，业务方最好读 |

### 泳道图

```
       #1              #2           #3              #4
前端 │ [订单列表]        [详情]──TRIGGER─►[订单详情]    [验收-张三]
     │      │AUTO                        │AUTO
后端 │ [GET /api/order/list]         [GET /api/order/{id}]      [POST /api/...]
     │      │CALL                         │CALL
资源 │ [GET redis][SELECT][SETEX]     [GET redis][SELECT][SETEX]  ...
```

- 三条泳道颜色 = 层；列号 = 步序（第几次用户交互），不是真实时间
- 跨泳道的连线就是调用关系，一眼能看出“这一下操作打到了哪些服务/资源”
- 同一格内多个节点自动堆叠，泳道高度随内容自适应
- 节点位置是**算出来的**（不是自动布局），所以不会跳动；可拖拽平移/缩放

**关键点：泳道图不改存储格式、不改图模型。** 数据里本来就有 `step_no`（时序）和
`layer`（层）两个坐标，泳道图只是在渲染时把它们当作 x / y 用。构图接口额外返回
每个节点首次出现的步序 `step`（纯响应字段加法）。
