...
观测页已从「静态图」升级为**两栏实时控制台**：
- 左栏实时事件流（毫秒时间戳 + 层级色点 + 边类型 + 耗时 + 新事件高亮），靠新增的
  `GET /observe/events?afterId=` 增量接口轮询（用自增 id 做游标，同毫秒多条也不丢不重）
- 右栏图，**只在数据签名变化时重绘**（否则每秒重排会闪）
- ★ 观测台自身被排除采集（前端 `ignorePaths: ['/graph','/observe']` + 后端已排除 `/observe/**`），
  否则观测动作会污染被观测对象
